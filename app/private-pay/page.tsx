...
import { Send, ShieldCheck, Loader2, Wallet } from 'lucide-react'
...
export default function PrivatePayPage() {
    ...
    const handleDeposit = async () => {
        if (!amount) return alert("Please enter the deposit amount first.")
        ...
        try {
            ...
            // Vault 'deposit' Smart Contract Execution
    ...
    const handleSend = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!recipient || !amount || !txHash) return
        ...
        try {
            const res = await fetch('/api/relayer', {
                ...
                body: JSON.stringify({
                    action: 'private_pay',
                    payload: { txHash, recipient, amount, asset }
                })
            })
            const data = await res.json()
            setResult(data)
        } catch (error) {
            setResult({ success: false, message: 'Failed to connect to relayer node.' })
        } finally {
            setLoading(false)
        }
    }
        ...
        <main className="min-h-screen pt-32 pb-24 md:pb-32 px-6 flex items-center justify-center">
            <div className="w-full max-w-lg">
                <div className="text-center mb-10 animate-fade-in-up bg-background/30 backdrop-blur-md border border-border/20 p-8 rounded-3xl shadow-2xl mx-auto w-fit">
                    ...
                    <h1 className="text-3xl font-bold text-foreground mb-3 [text-shadow:_0_2px_10px_rgb(0_0_0_/_80%)]">Private Pay Flow</h1>
                    <p className="text-muted-foreground text-sm font-medium [text-shadow:_0_1px_5px_rgb(0_0_0_/_100%)] max-w-sm mx-auto">
                        Execute anonymous transfers globally using our highly secured Vault Relayer Technology. Deterministic traces are severed instantly.
                    </p>
                </div>
                    ...
                    <form onSubmit={handleSend} className="space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="space-y-2 w-full sm:w-1/3">
                                ...
                                <select 
                                    ...
                                    className="w-full bg-background/50 border border-border/40 rounded-lg px-4 py-3 text-foreground focus:outline-none focus:ring-2 focus:ring-foreground/20 transition-all font-mono text-sm appearance-none">
                        ...
                        <div className="flex flex-col gap-3 pt-4 border-t border-border/20">
                            ...
                            <Button 
                                type="submit" 
                                disabled={loading}
                                className="w-full py-6 text-base font-semibold bg-foreground text-background hover:bg-foreground/90 transition-all relative overflow-hidden group">
                                {loading ? (
                                    <Loader2 className="animate-spin" size={20} />
                                ) : (
                                    <>
                                        <ShieldCheck className="mr-2" size={18} />
                                        2. Execute Private Relay
                                    </>
                                )}
                            </Button>
                        </div>
                ...
                <p className="text-center text-xs text-muted-foreground mt-8 animate-fade-in-up delay-300">
                    By confirming, you agree to our specialized protocol mechanics that obfuscate sender identity natively.
                </p>
            </div>
        </main>
    )
